...
\begin{itemize}
  ...
  \item Comprensión y adquisición de los datos (video, sensores de rendimiento y datos
        antropométricos).
  \item Preparación y preprocesamiento de los datos, lo que incluyó la extracción de
...
del algoritmo genético sin que se comprometiera la integridad operativa del
sistema global.
...
\begin{itemize}
  \item Velocidad de obturación: se impuso un tiempo de exposición máximo de $t_{exp}
        ...
        los cuales alcanzan velocidades angulares máximas $\omega_{max}$ en los puntos
        muertos del ciclo de pedaleo. El desenfoque en estas fases introduce un error
...
sistema aborda la reconstrucción cinemática. A diferencia de enfoques
simplistas que calculan ángulos planos entre vectores 2D, este trabajo
implementa una formulación rigurosa basada en los estándares de la Sociedad
Internacional de Biomecánica (ISB) para evitar errores
cinemático y singularidades matemáticas \citep{Wu2002_ISB_Part1}.
...
construyen marcos de referencia ortonormales para cada segmento corporal (fémur
y tibia), abandonando la geometría euclidiana simple en favor del álgebra
vectorial tridimensional.

Para el segmento femoral, definimos el origen $O_f$ en el Centro Articular de
la Cadera (HJC). Se construye el eje longitudinal $\mathbf{Y}_f$ mediante el
...
Para resolver la orientación espacial, se define un eje auxiliar
$\mathbf{Z}_{temp}$ que une los epicóndilos lateral y medial, lo que permite
calcular el eje antero-posterior $\mathbf{X}_f$ mediante producto vectorial,
asegurando la ortogonalidad respecto al plano cuasi-frontal:
...
define con origen en $\mathbf{M}_{FE}$, utilizando el eje transmaleolar para la
orientación distal .

\subsubsection{Cálculo de la matriz de rotación y ángulos de Euler}
La cinemática angular de la rodilla se obtiene proyectando la orientación del
marco tibial sobre el espacio vectorial del fémur. Si definimos
${}^G\mathbf{R}_f$ y ${}^G\mathbf{R}_t$ como las matrices de rotación absoluta
de fémur y tibia respecto al sistema global, la matriz de orientación relativa
${}^f\mathbf{R}_t$ se calcula como:
...
para la rodilla \citep{GroodSuntay1983}. El ángulo de flexión/extensión ($\alpha$), variable
crítica para la optimización de la altura del sillín, se extrae mediante:

\begin{equation}
  \alpha_{flexion} = \arctan2(-r_{12}, r_{22})
\end{equation}

Esta formulación desacopla matemáticamente la flexión pura de los movimientos
parásitos de abducción y rotación axial, proporcionando una señal de entrada
...
convencional es la sustitución del filtrado determinista (Butterworth)  \citep{Winter2009_Biomechanics} por un
estimador estocástico recursivo. Dado que el modelo de detección de pose
introduce ruido de alta frecuencia ("jitter") y que el filtrado convencional
introduce latencia de fase inaceptable para aplicaciones en tiempo real, se
implementó un Filtro de Kalman Lineal basado en un modelo cinemático de
Aceleración Angular Constante (CA).

Para cada grado de libertad articular $\theta$, se define el vector de estado
$\mathbf{x}_k$ en el instante $k$, el cual incluye explícitamente la velocidad
...
El modelo de transición de estado a priori, que predice la cinemática en $k$
basándose en $k-1$ y las ecuaciones de movimiento de Newton, se rige por la
matriz de transición $\mathbf{F}$:

\begin{equation}
  \hat{\mathbf{x}}_{k|k-1} = \mathbf{F} \hat{\mathbf{x}}_{k-1|k-1}, \quad \text{donde} \quad \mathbf{F} = \begin{bmatrix} 1 & \Delta t & \frac{1}{2}\Delta t^2 \\ 0 & 1 & \Delta t \\ 0 & 0 & 1 \end{bmatrix}
\end{equation}

La incertidumbre del modelo se gestiona mediante la matriz de covarianza del
ruido del proceso $\mathbf{Q}$, diseñada para modelar la sobreaceleración
(\textit{jerk}) como un ruido blanco espectral con densidad $\Phi_s$. Esto
permite que el filtro se adapte dinámicamente a cambios bruscos de cadencia sin
perder la suavidad de la señal:
...
Finalmente, la actualización a posteriori corrige la predicción utilizando la
medición ruidosa de la red neuronal $\mathbf{z}_k$, ponderada por la Ganancia
de Kalman óptima $\mathbf{K}_k$. Esto resulta en vectores
cinemáticos $(\theta, \dot{\theta}, \ddot{\theta})$ suaves y sincronizados en
fase, listos para alimentar el modelo de dinámica inversa.
...
modelos de regresión multivariable (basados en estudios de Bassett y Heil) los
cuales correlacionaron la pose sagital con el área frontal efectiva. El modelo
...
Se empleó el método de Newton-Euler Recursivo \citep{Winter2009_Biomechanics}, resolviendo las ecuaciones de
movimiento desde el segmento distal (pie/pedal) hacia el proximal (cadera).
Este enfoque permite propagar las fuerzas de reacción del suelo medidas por los
sensores de potencia a través de la cadena cinética del ciclista.
...
sujeto. Se definen $m_i$ como la masa del segmento, $I_i$ como el momento de
inercia centroidal y $\mathbf{a}_{cm, i} = [a_{ix}, a_{iy}]^T$ como la
aceleración lineal del centro de masas, derivada de la cinemática filtrada en
la sección anterior.

\subsubsection{Dinámica del segmento distal (Pie)}
El análisis inicia en la interfaz pedal-pie. Las fuerzas externas
$\mathbf{F}_{pedal} = [F_{px}, F_{py}]^T$ se descomponen en el sistema global.
Aplicando la segunda ley de Newton, se calculan las fuerzas de reacción en el
tobillo ($\mathbf{F}_{ankle}$):
...
Para el equilibrio de momentos, se aplica la ecuación de Euler respecto al
centro de masas (CM) para eliminar el término de aceleración lineal del CM. El
torque neto muscular en el tobillo $M_{ankle}$ se despeja considerando los
momentos generados por las fuerzas de reacción y la inercia rotacional del
segmento:

\begin{equation}
  \begin{aligned}
    M_{ankle} = & \ I_{foot} \alpha_{foot} - M_{pedal}                              \\
                & - (x_{ankle}-x_{cm}) F_{ankle_y} + (y_{ankle}-y_{cm}) F_{ankle_x} \\
                & - (x_{pedal}-x_{cm}) F_{py} + (y_{pedal}-y_{cm}) F_{px}
  \end{aligned}
\end{equation}

\subsubsection{Propagación recursiva (Pierna y Muslo)}
Siguiendo el principio de acción y reacción (Tercera Ley de Newton), las
fuerzas y momentos ejercidos por el pie sobre la pierna son
$-\mathbf{F}_{ankle}$ y $-M_{ankle}$. Para el segmento de la pierna (shank),
las ecuaciones de equilibrio en la rodilla se formulan como:
...
busca minimizar para reducir el riesgo de lesión patelar, se calcula sumando
los torques inerciales y los producidos por las fuerzas articulares distales y
proximales respecto al CM de la pierna:

\begin{equation}
  \begin{aligned}
    M_{knee} = & \ I_{shank} \alpha_{shank} + M_{ankle}                                    \\
               & - [(x_{knee}-x_{cm}) F_{knee_y} - (y_{knee}-y_{cm}) F_{knee_x}]           \\
               & - [(x_{ankle}-x_{cm}) (-F_{ankle_y}) - (y_{ankle}-y_{cm}) (-F_{ankle_x})]
  \end{aligned}
\end{equation}

Este proceso se repite recursivamente para el segmento del muslo (thigh) para
obtener el torque de cadera $M_{hip}$.

\subsubsection{Cálculo de Potencia Articular}
Finalmente, la eficiencia biomecánica se cuantifica mediante la potencia
mecánica instantánea $P_{joint}$ generada o absorbida en cada articulación.
Esta métrica integra la salida cinética con la cinemática, calculándose como el
...
La integral de esta potencia a lo largo del ciclo de pedaleo constituye la
función objetivo $J_{power}$ que el algoritmo genético minimiza, buscando la
configuración geométrica que permita generar los mismos vatios externos con el
menor coste mecánico interno.
